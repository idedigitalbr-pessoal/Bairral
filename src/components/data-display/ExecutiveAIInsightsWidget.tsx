  ...
  const handleGenerateInsights = async () => {
    ...
    try {
      const res = await geminiService.getExecutiveInsights({
        ...
        overdueCount: 3,
        topCategories: [
          { name: 'Relações de Trabalho & Assédio', count: 48 },
          { name: 'Segurança do Paciente / Prática Assistencial', count: 32 },
          { name: 'Conflito de Interesses & Fraude', count: 18 },
        ],
        topUnits: [
          { name: 'Hospital Psiquiátrico - Unidade A', count: 34 },
          { name: 'Rede de Saúde Mental - CAPS / Reabilitação', count: 22 },
        ],
      });
      setInsights(res);
    } catch (err) {
